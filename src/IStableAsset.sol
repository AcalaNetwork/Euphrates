...
interface IStableAsset {
    ...
    event StableAssetSwapped(
        address indexed sender, uint32 indexed poolId, uint32 i, uint32 j, uint256 dx, uint256 minDY, uint32 assetLength
    );
    ...
    event StableAssetRedeemedMulti(
        address indexed sender, uint32 indexed poolId, uint256[] amounts, uint256 maxRedeemAmount
    );
